    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">
          {userName ? `Progression de ${userName}` : 'Votre progression'}
        </h1>
        <p className="text-sm text-[#5A6478]">
          {isLoading
            ? 'Chargement de votre progression…'
            : isAuthenticated && syncStatus === 'cloud'
              ? 'Progression synchronisée avec votre compte.'
              : isAuthenticated && syncStatus === 'error'
                ? 'Mode local actif — la synchronisation reprendra automatiquement.'
                : 'Progression enregistrée localement dans ce navigateur.'}
        </p>
      </div>
          ...
          <div className="space-y-3">
            {MODULES.map((mod) => {
              ...
              return (
                ...
                >
                  <ModuleIcon moduleSlug={mod.slug} size={24} className="h-6 w-6 shrink-0" />
